...
Add `crypto` to the `[dependencies]` section in your `alya.toml`:
...
| `new_config(name, count)` | `name = "World", count = 1` | `CryptoConfig` | Constructs a new configuration struct. |
| `core_format_greeting(name)` | `name` | `string` | Core formatter producing `Hello, {name}!`. |
| `core_format_custom(config)` | `config: CryptoConfig` | `string` | Formats greeting using prefix and name from config. |
